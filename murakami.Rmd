---
title: "2022年 東京ヤクルトスワローズ 村上 宗隆 (公式戦)"
author: "hokmts works"
output: 
...
#    theme:
    ...
    navbar:
      - { title: "Home", href: "https://eloquent-pixie-729d17.netlify.app/"}
    social: [ "twitter","menu"]
...
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  dplyr::select(.,単打,長打,本塁打,空振り,見逃し,内野ゴ,内野直,内野飛,外野飛,ファウル,ボール,失出塁,
                振逃,everything()) %>% 
...
(round(prop.table(prslt4,margin=1),4)*100) %>% 
  ...
  rename(.data = . ,　"四球" = "ボール") %>% 
  arrange(.,-空三振) %>% 
  dplyr::select(.,単打,長打,本塁打,空三振,見三振,内野ゴ,内野直,内野飛,外野飛,四球,失出塁,振逃,
                everything()) %>%
...
prslt6 <- table(pitch_wk01$球速帯,pitch_wk01$結果2) %>% 
  as.data.frame.array() %>% 
  .[,c(6,13,10,11,12,3,4,5,7,1,2,8,9)]
...
pitch_wk12h <- paste("投球データ(対右投手contact)")
ggplot11p <- 
...
pitch_wk14h <- paste("投球データ(対右投手swstr)")
ggplot13p <- 
...
```{r fig.align = 'center',fig.width=10,fig.height=65}
...
ggplot15p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.001)
ggplot15p + facet_wrap(~投手名, ncol=5)
```
...
ggplot16p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.001)
ggplot16p + facet_wrap(~投手名, ncol=5)
```
