---
title: "2022年 千葉ロッテマリーンズ オスナ (公式戦)"
author: "hokmts works"
output: 
...
rownames(prslt3) <- c("130km台","140km台","150km台","NA")
...
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  dplyr::select(.,単打,空振り,見逃し,内野ゴ,内野飛,外野飛,ファウル,ボール,
                everything()) %>%
  kbl(.,caption = prslt3_tabh ) %>%
  kable_minimal(full_width = F)
#
...
(round(prop.table(prslt5,margin=1),4)*100) %>% 
  ...
  rename(.data = . ,　"四球" = "ボール") %>% 
  arrange(.,-空三振) %>% 
  dplyr::select(.,単打,空三振,見三振,内野ゴ,内野飛,外野飛,四球,
                everything()) %>%
...
prslt6 <- table(pitch_wk01$球速帯,pitch_wk01$結果2) %>% 
  ...
  dplyr::select(.,単打,長打,本塁打,空振り,見逃し,内野ゴ,内野直,内野飛,外野飛,ファウル,ボール,
                everything()) 
rownames(prslt6) <- c("130km台","140km台","150km台","NA")
...
prslt7 <- table(pitch_wk04$球速帯,pitch_wk04$結果2) %>% 
  ...
  dplyr::select(.,単打,長打,本塁打,空三振,見三振,内野ゴ,内野直,内野飛,外野飛,四球,
                everything()) 
rownames(prslt7) <- c("130km台","140km台","150km台","NA")
