---
title: "2022年 北海道日本ハムファイターズ 石川直也 (公式戦)"
author: "hokmts works"
output: 
...
#投手名 =="田中将大" | 投手名 =="上沢直之" | 投手名 =="松井裕樹" | 投手名 =="増田達至" |
...
rownames(prslt3) <- c("119km以下","120km台","130km台","140km台","150km台","NA")
...
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  dplyr::select(.,単打,長打,空振り,見逃し,内野ゴ,内野飛,外野飛,ファウル,ボール,
                everything()) %>%
...
(round(prop.table(prslt4,margin=1),4)*100) %>% 
  ...
  rename(.data = . ,　"四球" = "ボール") %>% 
  arrange(.,-空三振) %>% 
  dplyr::select(.,単打,長打,空三振,見三振,内野ゴ,内野飛,外野飛,四球,
                everything()) %>%
...
prslt6 <- table(pitch_wk01$球速帯,pitch_wk01$結果2) %>% 
  ...
  dplyr::select(.,単打,長打,本塁打,空振り,見逃し,内野ゴ,内野飛,外野飛,ファウル,ボール,
                everything()) 
rownames(prslt6) <- c("119km以下","120km台","130km台","140km台","150km台","NA")
...
```{r fig.align = 'center',fig.width=10,fig.height=40}
