...
#村上宗隆,大島洋平,佐野恵太,宮﨑敏郎,ビシエド
#近本光司,岡林勇希,牧秀悟,坂倉将吾,吉川尚輝
#中野拓夢,塩見泰隆,丸佳浩,マクブルーム
#オスナ,阿部寿樹,大山悠輔,小園海斗
#佐藤輝明,菊池涼介,桑原将志,岡本和真
#糸原健斗,木下拓哉,山田哲人,長岡秀樹
#ポランコ
...
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  dplyr::select(.,単打,長打,空振り,見逃し,内野ゴ,内野直,内野飛,外野飛,ファウル,ボール,
                everything()) %>% 
  kbl(.,caption = prslt3_tabh ) %>%
  kable_minimal(full_width = F)
#
...
(round(prop.table(prslt5,margin=1),4)*100) %>% 
  ...
  rename(.data = . ,　"見三振" = "見逃し") %>% 
  rename(.data = . ,　"四球" = "ボール") %>% 
  dplyr::select(.,単打,長打,空三振,見三振,内野ゴ,内野直,内野飛,外野飛,四球,
                everything()) %>%
  arrange(.,-空三振) %>% 
