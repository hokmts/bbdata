---
title: "2022年 福岡ソフトバンクホークス モイネロ (公式戦)"
author: "hokmts works"
output: 
  flexdashboard::flex_dashboard:
    ...
    navbar:
      - { title: "Home", href: "https://hokmts23.netlify.app/en/2022players/"}
    social: [ "twitter","menu"]
#default”, “bootstrap”, “cerulean”, “cosmo”, “darkly”, “flatly”, “journal”, “lumen”, “paper”, #“readable”, “sandstone”, “simplex”, “spacelab”, “united”, “yeti”
#投手名 =="山本由伸" | 投手名 =="千賀滉大" | 投手名 =="上沢直之" | 投手名 =="加藤貴之" |
#投手名 =="髙橋光成" | 投手名 =="伊藤大海" | 投手名 =="小島和哉" | 投手名 =="宮城大弥" |
#投手名 =="田中将大" | 投手名 =="松井裕樹" | 投手名 =="増田達至" |
#投手名 =="平野佳寿" | 投手名 =="益田直也" | 投手名 =="モイネロ" | 投手名 =="オスナ" |
#投手名 =="北山亘基" | 投手名 =="森唯斗" | 投手名 =="石川直也" | 投手名 =="平良海馬" |
#投手名 =="水上由伸" | 投手名 =="東條大樹" | 投手名 =="西口直人" | 投手名 =="嘉弥真新也" |
#投手名 =="藤井皓哉" | 投手名 =="ゲレーロ" | 投手名 =="本田圭佑" | 投手名 =="宋家豪" |
#投手名 =="阿部翔太" 
---
...
pitch_wk01 <- filter(pitch_data, 投手名 == 投手名10s )
# data numeric 変換
...
#
# 投手名10a 全投球内容 散布図(右打者)
pitch_wk02h <- paste("投球データ (対右打者)")
pitch_wk02 <- filter(pitch_wk01, 打者左右 == "右")
ggplot02p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.001) 
#
# 投手名10a 全投球内容 散布図(左打者)
pitch_wk03h <- paste("投球データ (対左打者)")
pitch_wk03 <- filter(pitch_wk01, 打者左右 == "左")
ggplot03p <- 
...
ggplot04p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.001)
#
# 投手名10a 全投球内容 散布図(結果球)
pitch_wk05h <- paste("投球データ (対右打者)")
...
```{r }
pitch_wk07h <- paste("投球データ (対右打者結果球)")
ggplot07w <- table(pitch_wk05$投手名,pitch_wk05$結果) %>% 
  as.data.frame(.) %>% 
  ggplot(.,aes(x = reorder(Var2,-Freq)  , y = Freq ,fill= Var2 )) + 
  geom_col() + scale_fill_viridis_d() + geom_text(aes(label=Freq),vjust=-0.2,size=3) +
  ...
  xlab("投球結果") + ylab("結果数")
#
pitch_wk08h <- paste("投球データ (対左打者結果球)")
...
rownames(p_rh_tab) <- "右打者"
rownames(p_lh_tab) <- "左打者"
...
rownames(prslt3) <- c("120km台","130km台","140km台","150km台","NA")
...
p_type_tab <- table(pitch_wk01$球種,pitch_wk01$打者チーム) %>%
...
# 打者別 {data-navmenu="球種構成"}
```{r }
p_type_tabh <- paste("球種構成(%)")
p_type_tab <- table(pitch_wk01$球種,pitch_wk01$打者名) %>%
  t()　%>% as.data.frame.array()
球数合計 <- rowSums(p_type_tab)
p_type_tab1 <- (p_type_tab / 球数合計) 
round((p_type_tab1) *100,2) %>% 
...
swing <- table(pitch_wk01$結果5,pitch_wk01$打者チーム)
contact <- table(pitch_wk01$結果7,pitch_wk01$打者チーム)
swstr <- table(pitch_wk01$結果6,pitch_wk01$打者チーム)
k_sum <- table(pitch_wk01$打者チーム)
# per 算出
swing_per <- swing[1,] / k_sum
contact_per <- contact[1,] /  swing[1,]
swstr_per <- swstr[1,] / k_sum
...
colnames(sw_data) <- c("swing(%)","contact(%)","swstr(%)","whiff(%)") 
# 球速 data kable　作成
p_max <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者チーム), 
                   FUN=max,na.rm = TRUE)
p_mean <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者チーム), 
                    FUN=mean,1,na.rm = TRUE)
p_medi <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者チーム),
                    FUN=median,na.rm = TRUE)
p_min <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者チーム), 
                   FUN=min,na.rm = TRUE)
p_ver <- cbind(p_max[2],p_mean[2],p_min[2])
...
# 打者別 {data-navmenu="swing"}
```{r }
row_sum <- nrow(pitch_wk01)
swing <- table(pitch_wk01$結果5,pitch_wk01$打者名)
contact <- table(pitch_wk01$結果7,pitch_wk01$打者名)
swstr <- table(pitch_wk01$結果6,pitch_wk01$打者名)
k_sum <- table(pitch_wk01$打者名)
# per 算出
swing_per <- swing[1,] / k_sum
contact_per <- contact[1,] /  swing[1,]
swstr_per <- swstr[1,] / k_sum
...
colnames(sw_data) <- c("swing(%)","contact(%)","swstr(%)","whiff(%)") 
# 球速 data kable　作成
p_max <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者名), 
                   FUN=max,na.rm = TRUE)
p_mean <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者名), 
                    FUN=mean,1,na.rm = TRUE)
p_medi <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者名),
                    FUN=median,na.rm = TRUE)
p_min <- aggregate(x=pitch_wk01[c("球速")], by=list(pitch_wk01$打者名), 
                   FUN=min,na.rm = TRUE)
p_ver <- cbind(p_max[2],p_mean[2],p_min[2])
p_ver_tabh <- paste("対打者別")
colnames(p_ver) <- c("球速(max)","球速(ave)","球速(min)") 
...
```{r }
prslt3_tabh <- paste("対右打者(%)")
prslt2 <- table(pitch_wk02$結果2,pitch_wk02$球種) %>% 
  t() %>% as.data.frame.array()
prslt3 <- prslt2 / rowSums(prslt2)
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  dplyr::select(.,単打,長打,本塁打,空振り,見逃し,内野ゴ,内野飛,外野飛,ファウル,ボール,
                everything()) %>%
  kbl(.,caption = prslt3_tabh ) %>%
  kable_minimal(full_width = F)
#
prslt3_tabh <- paste("対左打者(%)")
prslt2 <- table(pitch_wk03$結果2,pitch_wk03$球種) %>% 
  t() %>% as.data.frame.array()
prslt3 <- prslt2 / rowSums(prslt2)
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  dplyr::select(.,単打,本塁打,空振り,見逃し,内野ゴ,内野飛,外野飛,ファウル,ボール,
                everything()) %>%
  kbl(.,caption = prslt3_tabh ) %>%
  kable_minimal(full_width = F)
#
prslt4_tabh <- paste("対右打者/結果球(%)")
...
(round(prop.table(prslt4,margin=1),4)*100) %>% 
  ...
  rename(.data = . ,　"四球" = "ボール") %>% 
  arrange(.,-空三振) %>% 
  dplyr::select(.,単打,長打,本塁打,空三振,見三振,内野ゴ,内野飛,外野飛,四球,
                everything()) %>%
  kbl(.,caption = prslt4_tabh ) %>%
  kable_minimal(full_width = F)
#
prslt5_tabh <- paste("対左打者/結果球(%)")
...
(round(prop.table(prslt5,margin=1),4)*100) %>% 
  ...
  rename(.data = . ,　"四球" = "ボール") %>% 
  arrange(.,-空三振) %>% 
  dplyr::select(.,単打,本塁打,空三振,見三振,内野ゴ,内野飛,外野飛,四球,
                everything()) %>%
...
prslt6 <- table(pitch_wk01$球速帯,pitch_wk01$結果2) %>% 
  ...
  dplyr::select(.,単打,長打,本塁打,空振り,見逃し,内野ゴ,内野飛,外野飛,ファウル,ボール,
                everything()) 
rownames(prslt6) <- c("120km台","130km台","140km台","150km台","NA")
...
prslt7 <- table(pitch_wk04$球速帯,pitch_wk04$結果2) %>% 
  ...
  dplyr::select(.,単打,長打,本塁打,空三振,見三振,内野ゴ,内野飛,外野飛,四球,
                everything()) 
rownames(prslt7) <- c("120km台","130km台","140km台","150km台","NA")
...
prslt8 <- table(pitch_wk01$結果2,pitch_wk01$打者チーム) %>% 
  t() %>% as.data.frame.array()
...
prslt9 <- table(pitch_wk04$結果2,pitch_wk04$打者チーム) %>% 
  t() %>% as.data.frame.array()
...
# 打者別 {data-navmenu="投球結果"}
```{r }
prslt10_tabh <- paste("対打者別(%)")
prslt10 <- table(pitch_wk01$結果2,pitch_wk01$打者名) %>% 
  t() %>% as.data.frame.array()
...
# 打者別結果球 {data-navmenu="投球結果"}
```{r }
prslt11_tabh <- paste("対打者別結果球(%)")
prslt11 <- table(pitch_wk04$結果2,pitch_wk04$打者名) %>% 
  t() %>% as.data.frame.array()
...
pitch_wk11h <- paste("投球データ(対右打者contact)")
pitch_wk12 <- filter(pitch_wk03, 結果7 == "contact" )
pitch_wk12h <- paste("投球データ(対左打者contact)")
ggplot11p <- 
...
pitch_wk13h <- paste("投球データ(対右打者swstr)")
pitch_wk14 <- filter(pitch_wk03, 結果6 == "swstr"  )
pitch_wk14h <- paste("投球データ(対左打者swstr)")
ggplot13p <- 
...
pitch_wk15h <- paste("投球データ(対右打者被安打)")
ggplot15p <- 
...
pitch_wk16h <- paste("投球データ(対左打者被安打)")
ggplot16p <- 
...
# 球種内容 {data-navmenu="打者別投球"}

```{r fig.align = 'center',fig.width=10,fig.height=45}
...
ggplot17p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.001)
ggplot17p + facet_wrap(~打者名, ncol=5)
```

Column {data-width=350}
-----------------------------------------------------------------------

# 投球結果 {data-navmenu="打者別投球"}
...
ggplot18p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.001)
ggplot18p + facet_wrap(~打者名, ncol=5)
```
