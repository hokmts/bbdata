...
#    theme:
    ...
    navbar:
      - { title: "Home", href: "https://fabulous-wisp-447b22.netlify.app/"}
    social: [ "twitter","menu"]
...
ggplot01p <- 
  ...
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.003) 
#
...
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  .[,c(6,13,9,11,12,3,4,5,7,1,2,8,10)] %>%
...
round((prslt3) *100,2) %>% 
  arrange(.,-空振り) %>% 
  .[,c(6,14,10,12,13,3,4,5,7,1,2,8,9,11)] %>% 
  kbl(.,caption = prslt3_tabh ) %>%
  kable_minimal(full_width = F)
#
...
(round(prop.table(prslt5,margin=1),4)*100) %>% 
  .[,c(5,13,9,11,12,2,3,4,6,1,7,8,10)] %>% 
  as.data.frame.array() %>% 
  rename(.data = . ,　"空三振" = "空振り") %>% 
  rename(.data = . ,　"見三振" = "見逃し") %>% 
  rename(.data = . ,　"四球" = "ボール") %>% 
...
pitch_wk12h <- paste("投球データ(対右打者contact)")
ggplot11p <- 
...
pitch_wk14h <- paste("投球データ(対右打者swstr)")
ggplot13p <- 
...
ggplot15p <- 
  ...
  theme(axis.ticks = element_blank(),axis.text.y = element_blank(),
        axis.text.x = element_blank(),axis.title.x = element_text(size = 10)) +
  xlab(pitch_wk15h) + ylab("") +
  geom_vline(xintercept = 75) +
  geom_hline(yintercept = 90) +
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.003)
ggplot15p + facet_wrap(~打者名, ncol=5)
```
...
ggplot16p <- 
  ...
  theme(axis.ticks = element_blank(),axis.text.y = element_blank(),
        axis.text.x = element_blank(),axis.title.x = element_text(size = 10)) +
  xlab(pitch_wk16h) + ylab("") +
  geom_vline(xintercept = 75) +
  geom_hline(yintercept = 90) +
  geom_rect(aes(xmin = 30, xmax = 120, ymin = 25, ymax = 150),
            fill = "palegreen", colour = "black", alpha = 0.003)
ggplot16p + facet_wrap(~打者名, ncol=5)
```
